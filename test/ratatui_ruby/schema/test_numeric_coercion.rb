...
class TestNumericCoercion < Minitest::Test
  ...
  def test_shape_point_coerces_floats
    point = RatatuiRuby::Shape::Point.new(x: DuckFloat.new(1.5), y: DuckFloat.new(2.5))
    assert_equal 1.5, point.x
    assert_equal 2.5, point.y
  end

  def test_shape_point_coerces_rationals
    point = RatatuiRuby::Shape::Point.new(x: Rational(3, 2), y: Rational(5, 2))
    assert_equal 1.5, point.x
    assert_equal 2.5, point.y
  end

  def test_shape_point_rejects_nil
    assert_raises(TypeError) { RatatuiRuby::Shape::Point.new(x: nil, y: 0) }
    assert_raises(TypeError) { RatatuiRuby::Shape::Point.new(x: 0, y: nil) }
  end

  def test_shape_line_coerces_floats
    line = RatatuiRuby::Shape::Line.new(
  ...
  def test_shape_rectangle_coerces_floats
    rect = RatatuiRuby::Shape::Rectangle.new(
  ...
  def test_shape_circle_coerces_floats
    circle = RatatuiRuby::Shape::Circle.new(
  ...
  def test_shape_label_coerces_floats
    label = RatatuiRuby::Shape::Label.new(
  ...
  def test_gauge_ratio_coerces_float
    gauge = RatatuiRuby::Gauge.new(ratio: Rational(1, 2))
    assert_equal 0.5, gauge.ratio
  end

  def test_gauge_percent_coerces_float
    gauge = RatatuiRuby::Gauge.new(percent: DuckFloat.new(75))
    assert_equal 0.75, gauge.ratio
  end

  def test_line_gauge_ratio_coerces_float
    gauge = RatatuiRuby::LineGauge.new(ratio: Rational(1, 4))
    assert_equal 0.25, gauge.ratio
  end

  def test_center_percentages_coerce_floats
    center = RatatuiRuby::Center.new(
      child: RatatuiRuby::Paragraph.new(text: "Test"),
  ...
  def test_line_chart_bounds_coerce_floats
    chart = RatatuiRuby::LineChart.new(
      datasets: [],
  ...
  def test_rect_coerces_floats_to_integers
    rect = RatatuiRuby::Rect.new(x: 10.9, y: 5.1, width: 80.5, height: 24.9)
    assert_equal 10, rect.x
    assert_equal 5, rect.y
    assert_equal 80, rect.width
    assert_equal 24, rect.height
  ...
  def test_cursor_coerces_integers
    cursor = RatatuiRuby::Cursor.new(x: DuckInt.new(5), y: DuckInt.new(10))
  ...
  def test_draw_cell_coerces_integers
    cell = RatatuiRuby::Cell.new(char: "X")
  ...
  def test_list_selected_index_coerces_or_nil
    list_nil = RatatuiRuby::List.new(items: [], selected_index: nil)
    assert_nil list_nil.selected_index

    list_duck = RatatuiRuby::List.new(items: [], selected_index: DuckInt.new(5))
    assert_equal 5, list_duck.selected_index
  end

  def test_list_optional_params_reject_false
    assert_raises(TypeError) { RatatuiRuby::List.new(items: [], selected_index: false) }
  end
  ...
  def test_table_optional_params_allow_nil
    table = RatatuiRuby::Table.new(rows: [], selected_row: nil, selected_column: nil)
  ...
  def test_bar_chart_max_allows_nil
    chart = RatatuiRuby::BarChart.new(data: { "A" => 10 }, max: nil)
    assert_nil chart.max
  end

  def test_bar_value_coerces_integer
    bar = RatatuiRuby::BarChart::Bar.new(value: DuckInt.new(42))
  ...
  def test_sparkline_allows_nil_in_data
    sparkline = RatatuiRuby::Sparkline.new(data: [1, nil, 3], max: nil)
  ...
  def test_constraint_factory_methods_coerce_integers
    assert_equal 10, RatatuiRuby::Constraint.length(DuckInt.new(10)).value
    assert_equal 50, RatatuiRuby::Constraint.percentage(DuckInt.new(50)).value
    assert_equal 5, RatatuiRuby::Constraint.min(DuckInt.new(5)).value
    assert_equal 20, RatatuiRuby::Constraint.max(DuckInt.new(20)).value
    assert_equal 2, RatatuiRuby::Constraint.fill(DuckInt.new(2)).value
    assert_equal [1, 3], RatatuiRuby::Constraint.ratio(DuckInt.new(1), DuckInt.new(3)).value
  end
  ...
  def test_block_padding_coerces_integers
    block_uniform = RatatuiRuby::Block.new(padding: DuckInt.new(5))
    assert_equal 5, block_uniform.padding

    block_array = RatatuiRuby::Block.new(
