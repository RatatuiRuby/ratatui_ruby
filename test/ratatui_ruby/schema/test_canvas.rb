...
class TestCanvas < Minitest::Test
  def test_point_creation
    p = RatatuiRuby::Point.new(x: 1.0, y: 2.0)
    assert_equal 1.0, p.x
    assert_equal 2.0, p.y
  end

  def test_line_creation
    l = RatatuiRuby::Line.new(x1: 0.0, y1: 0.0, x2: 10.0, y2: 10.0, color: "red")
  ...
  def test_rectangle_creation
    r = RatatuiRuby::Rectangle.new(x: 0.0, y: 0.0, width: 10.0, height: 10.0, color: "blue")
  ...
  def test_circle_creation
    c = RatatuiRuby::Circle.new(x: 5.0, y: 5.0, radius: 2.5, color: "green")
  ...
  def test_map_creation
    m = RatatuiRuby::Map.new(color: "yellow", resolution: :high)
  ...
  def test_canvas_creation
    shapes = [
      RatatuiRuby::Line.new(x1: 0.0, y1: 0.0, x2: 1.0, y2: 1.0, color: "white"),
    ]
