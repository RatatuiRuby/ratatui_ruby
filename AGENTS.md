...
-   **Format:**
    ...
    -   Body: Explanation if necessary (wrap at 72 chars).
        ...
        -   **DO NOT list the files changed or the edits made in the body.**
        -   Do not use markdown, except as required in AI Attribution.
...
1.  **Default Rake Task Passes:** Run `bundle exec rake || echo "FAIL"` (the default task) to execute **ALL** tests and linting. Do not rely on partial test runs, or `rake test`, or `rake lint` alone. Confirm it passes with no new errors **or warnings**.
