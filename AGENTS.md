...
-   **AI Attribution:**
    -   If you (an AI agent) generated the code, search the git log for your name to find the correct `Generated with` and `Co-Authored-By` format.
-   **Sourcehut Tickets:**
...
1.  **Tests & Linting Pass:** Run `rake` and confirm it passes.
2.  **Documentation Updated:** If code behavior changed, update relevant `doc/` files, `README.md`, and/or `ratatui_ruby-wiki` files,.
3.  **Commit Message Suggested:** You **MUST** ensure the final message to the user includes a suggested commit message block. This is NOT optional.
