...
1.  **Default Rake Task Passes:** Run `bundle exec rake` (the default task) to execute **ALL** tests and linting. Do not rely on partial test runs, or `rake test`, or `rake lint` alone. Confirm it passes with no new errors **or warnings**.
