...
- **Format:**
    ...
    - Body: Explanation if necessary (wrap at 72 chars).
        ...
        - **DON'T use markdown syntax** (no backticks, no bolding, no lists, no links), except as required in AI Attribution. The commit message must be plain text.
    - Footer: AI attribution if generated by an agent, sourcehut ticket if implemented by a ticket, or both. Search `git log` for yourself for the format. Must include "Generated with" and "Co-Authored-By:".
