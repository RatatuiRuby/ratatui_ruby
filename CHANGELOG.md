...
- **Block Border Set**: Added `border_set` parameter to `Block` widget, allowing custom characters for borders. This enables using digits, specific symbols, or other characters for border lines, as demonstrated in the `box_demo`. Note that unique border characters are interned, so avoid generating infinite unique sets.
