...
- [x] Add `item` alias to `WidgetFactories`
- [x] Add terse shape aliases to `CanvasFactories` (except terse `rectangle` - conflicts with Rect)
- [ ] Add `shape(type, ...)` dispatcher (nice-to-have)
- [ ] Add `constraint(type, ...)` dispatcher (nice-to-have)
- [x] Add bidirectional shape aliases (`*_shape`) including `rectangle_shape`
- [ ] Add `text(type, ...)`, `widget(type, ...)`, `state(type, ...)` dispatchers (nice-to-have)
- [x] Update RBS signatures for all new methods
- [x] Update RDoc for all new methods
...
- [x] Add `percent` to `Gauge` and `LineGauge`
- [x] ~~Add `selection` alias~~ **Rejected** - ambiguous (returns item or index?)
- [x] Add `selected_item` to `List`
